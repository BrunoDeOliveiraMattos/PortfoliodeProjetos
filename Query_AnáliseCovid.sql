-- Análise Exploratória de Dados da COVID-19
...
select location, date, total_deaths, total_cases, ((total_deaths/total_cases) *100) as 'Porcentagem de Mortes'
from [Projeto 1 ].dbo.Mortes_Covid
where continent is not null
...
select Max(total_cases) as TotaldeNovosCasos, Max(total_deaths) as TotaldeMortes, (Max(total_deaths)  / Max(total_cases))*100  as PorcentagemDeMorte
from [Projeto 1 ].dbo.Mortes_Covid
...
-- Relação entre taxa de vacinação e novas mortes com o passar do tempo no mundo
...
--Países com Maior taxa de mortalidade e maior taxa de vacinação
Select dea.continent,dea.location, Max(dea.total_deaths), Max(vac.total_vaccinations)
from [Projeto 1 ].dbo.Mortes_Covid dea --abreviação para não precisar reescrever a base toda vez que for usar a fórmula
Join [Projeto 1 ].dbo.VacinaçãoCovid vac --abreviação para não precisar reescrever a base toda vez que for usar a fórmula
	on dea.location = vac.location
	and dea.date = vac.date
